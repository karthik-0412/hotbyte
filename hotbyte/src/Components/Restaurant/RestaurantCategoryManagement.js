...
const RestaurantCategoryManagement = () => {
  ...
  const handleSave = async () => {
    try {
      if (editing) {
        await axios.put(`${API}/${formData.categoryId}`, formData);
      } else {
        await axios.post(`${API}/add`, { ...formData, createdAt: new Date().toISOString() });
      }
  ...
  const handleToggleStatus = async (cat) => {
    try {
      ...
      await axios.put(`${API}/${cat.categoryId}/${endpoint}`);
      fetchCategories();
    } catch (err) {
      console.error(err);
    }
